...
public class CascadingExcavations {
    public static final EventManager.ISubscribe featManager = EventManager.create(BlockEvent.BreakEvent.class, EventManager.Bus.FORGE)
            .filter(event -> EposCharacterUtil.hasFeat(event.getPlayer(), new ResourceLocation(Epos.ID, "cascading_excavations")))
            .process(event -> {
                ItemStack stack = event.getPlayer().getMainHandItem();
                if (event.getWorld() instanceof World) {
                    EposBlockUtil.handleCascadingBlockDestruction(stack, (World) event.getWorld(), event.getState(), event.getPos(), event.getPlayer(), Tags.Blocks.ORES, 125, 25);
                }
            });
}
