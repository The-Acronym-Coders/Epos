...
package com.teamacronymcoders.epos.data;

import com.teamacronymcoders.epos.data.client.EposLocalizationProvider;
...
public class EposData {
    ...
    /**
     * Initialize any information related to data generators for this mod.
     *
     * @param modBus the mod event bus
     */
    public static void init(IEventBus modBus) {
        modBus.addListener(EposData::gatherData);
    }

    /**
     * An event that listens for when providers are being added to the
     * {@link DataGenerator}.
     *
    ...
    private static void gatherData(GatherDataEvent event) {
        ...
        addProvider(generator, output -> new PackMetadataGenerator(output)
                .add(PackMetadataSection.TYPE, new PackMetadataSection(
                        Component.translatable(EposLocalizationKeys.PACK_DESCRIPTION),
                        DetectedVersion.BUILT_IN.getPackVersion(PackType.CLIENT_RESOURCES.bridgeType),
        ...
        if (event.includeClient()) {
            addProvider(generator, EposLocalizationProvider::new);
        }
    }
